...
import { streamClassification } from '@/server/classification-action'
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: aiMessageId,
        role: 'ai',
        content: 'Classifying your text...',
        isClassification: true
      }])

      // Start streaming classification
      const stream = await streamClassification(userText)
                    ...
                    <div
                        ...
                        className={`mb-4 p-4 rounded-lg ${
                            message.role === 'user'
                                ...
                                : message.isClassification 
                                  ? 'border-green-400 border ml-auto max-w-[80%]' 
                                  : 'border-orange-400 border ml-auto max-w-[80%]'
                        }`}
                    >
                      <div className="text-sm font-semibold mb-1">
                        {message.role === 'user' ? 'You:' : message.isClassification ? 'Classification:' : 'AI:'}
                      </div>
          ...
          <div className="max-w-2xl mx-auto p-4">
            <form onSubmit={handleSubmit} className="flex gap-2">
              ...
              <button
                  ...
                  disabled={isStreaming}
              >
                {isStreaming ? 'Classifying...' : 'Classify'}
              </button>
            </form>
          </div>
        </div>
